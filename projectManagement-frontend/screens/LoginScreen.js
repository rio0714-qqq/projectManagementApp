import * as React from 'react';
import { useState } from 'react';
import { View, Text, TextInput, StyleSheet, TouchableOpacity } from 'react-native';
import axios from 'axios';
import API_BASE_URL from '../config';

/**
 * LoginScreen.js
 * ログイン画面
 */

export function LoginScreen({ navigation }) {

  // State管理
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');

  const handlelogin = async () => {
    try {
      // 【API通信】POST ログイン
      const loginResponse = await axios.post(`${API_BASE_URL}/api/v1/auth/login`, {
        email,
        password,
      },
        {
          // Json形式に変換
          headers: {
            'Content-Type': 'application/json'
          }
        });
      console.log("ログインログ", loginResponse.data);

      //【API通信】POST 認証
      const authResponse = await axios.get(`${API_BASE_URL}/api/v1/auth/me`, {
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${loginResponse.data.token}`
        }
      });
      // ログイン成功で案件管理画面に遷移
      if (authResponse.data.data.role == "admin") {
        navigation.navigate('CaseManagementScreen', { loginResData: loginResponse.data });
      } else {
        navigation.navigate('UserCaseManagementScreen', { loginResData: loginResponse.data });
      }



    } catch (error) {
      console.error(error);
    }
  };

  return (
    <View style={styles.container}>
      <View style={styles.contentsContainer}>
        <View style={styles.titleContainer}>
          <Text style={styles.title}>ログイン画面</Text>
        </View>
        <View>
          <Text style={styles.text}>メールアドレス:</Text>
          <TextInput
            style={styles.input}
            value={email}
            onChangeText={setEmail}
            keyboardType="email-address"
            autoCapitalize="none"
            placeholder="email address"
          />
        </View>
        <View>
          <Text style={styles.text}>パスワード:</Text>
          <TextInput
            style={styles.input}
            secureTextEntry
            value={password}
            onChangeText={setPassword}
            placeholder="password"
          />
        </View>
        <TouchableOpacity style={styles.button} onPress={handlelogin}>
          <Text style={styles.buttonText}>ログイン</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
}

const styles = StyleSheet.create({
  container: {
    flex: 1,
    backgroundColor: '#faedf4',
    alignItems: 'center',
    justifyContent: 'center',
  },
  contentsContainer: {
    width: '80%',
  },
  titleContainer: {
    marginBottom: 10
  },
  title: {
    fontSize: 30,
    fontWeight: 'bold',
    color: '#8783a6'
  },
  text: {
    fontWeight: 'bold',
    color: '#8783a6',
    fontSize: 15,
    marginBottom: 5
  },
  input: {
    borderColor: '#8783a6',
    borderWidth: 3,
    marginBottom: 16,
    padding: 8,
    backgroundColor: '#ece8f6',
    color: '#8783a6',
    borderRadius: 3
  },
  button: {
    marginTop: 22,
    borderColor: '#8783a6',
    backgroundColor: '#8783a6',
    padding: 9,
    alignItems: 'center',
    justifyContent: 'center',
    borderRadius: 5
  },
  buttonText: {
    fontWeight: 'bold',
    color: '#ece8f6',
    fontSize: 18,
  }
});
